...
var shiftSegments = function () {
  ...
  var costReductionChrome = roundToTwo(costReduction * 0.465);
  var costReductionFire = roundToTwo(costReduction * 0.0601);
  var costReductionEdge = roundToTwo(costReduction * 0.0351);


  var updateSavingsData = function (costReductionChrome,costReductionFire,costReductionEdge) {
      document.getElementById('cost-reduction-chrome').innerHTML = costReductionChrome;
      document.getElementById('cost-reduction-fire').innerHTML = costReductionFire;
      document.getElementById('cost-reduction-edge').innerHTML = costReductionEdge;
  };

  updateRequestData(selectedRequestData, compareRequestedData, savingsPercentage, costReduction);
  updateSavingsData(costReductionChrome,costReductionFire,costReductionEdge);
...
getBrowser = function() {
  var t = !!window.opr && !!opr.addons || !!window.opera || navigator.userAgent.indexOf(" OPR/") >= 0,
      ...
      o = !r && /Edg/i.test(navigator.userAgent),
      a = /Google Inc/.test(navigator.vendor),
      s = /Chrome/i.test(navigator.userAgent) && /android/i.test(navigator.userAgent);
  return t ? BROWSER.OPERA : e ? BROWSER.FIREFOX : i ? BROWSER.SAFARI : r ? BROWSER.IE : o ? BROWSER.EDGE : n ? BROWSER.SAFARI : a || s ? BROWSER.CHROME : BROWSER.UNKNOWN
};
...
var multicodec = function () {
  ...
  if (browser === BROWSER.CHROME || browser === BROWSER.FIREFOX) {
    ...
    document.getElementById('detected-browser2').innerHTML = '<i class="fa fa-chrome browser-icon" aria-hidden="true"></i>' + " and ";
    document.getElementById('detected-browser3').innerHTML = '<i class="fa fa-firefox browser-icon" aria-hidden="true"></i>';
  } else if (browser === BROWSER.EDGE) {
    selectedCodec = CODEC.H265;
    source.dash = '//bitmovin-a.akamaihd.net/webpages/demos/content/multi-codec/hevc/stream.mpd';
    document.getElementById('detected-browser2').innerHTML = '<i class="fa fa-edge browser-icon" aria-hidden="true"></i>';
